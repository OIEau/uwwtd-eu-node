...
Drupal.behaviors.openlayers_behavior_ole = {
  'attach': function(context, settings) {
    ...
    if (data) {
      ...
      processing_controls.push("TransformFeature");
      other_controls.push("UploadShape");
      // other_controls.push("UndoRedo");
      ...
      editor.editLayer.events.register('featureuploaded', this, openlayers_behavior_ole_update);
      var wkt ='';
      ...
      $(data.openlayers.div).after("<input style='padding:6px;float:right;border: 2px solid #5794e7;font-weight:bold' id='uploadShapeButton' type='button' value='Transférer une géométrie (.shp ou .geojson)' />");
      $('#uploadShapeButton').on('click', function() {
        var uploadShapeControl = new OpenLayers.Editor.Control.UploadFeature(data.openlayers.getLayersByName('Editor')[0]);
        uploadShapeControl.map = data.openlayers;
        editor.controls.UploadShape = uploadShapeControl;
        editor.controls.UploadShape.activate();
      });
    }
  }
};
